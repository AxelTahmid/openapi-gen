...
func NewSchemaGenerator(opts ...*TypeIndex) *SchemaGenerator {
	slog.Info("[openapi] NewSchemaGenerator: initializing", "opts_len", len(opts))
	var idx *TypeIndex
...
func (sg *SchemaGenerator) GenerateSchema(typeName string) *Schema {
	slog.Info("[openapi] GenerateSchema: called", "typeName", typeName)

	// 1) Fast-path simple/empty types
	if typeName == "" || typeName == "<autogenerated>" {
		slog.Info("[openapi] GenerateSchema: empty typeName, returning object schema")
	...
	if sg.typeIndex != nil {
		if schema, ok := sg.typeIndex.externalKnownTypes[typeName]; ok {
			slog.Info("[openapi] GenerateSchema: using externalKnownTypes", "typeName", typeName)
			return schema
		}
	}
	...
	if _, exists := sg.schemas[typeName]; exists {
		sg.mutex.Unlock()
		slog.Info("[openapi] GenerateSchema: schema already exists", "typeName", typeName)
		...
		if len(parts) == 2 {
			...
			if ts := sg.typeIndex.LookupType(pkg, typ); ts != nil {
				if structType, ok := ts.Type.(*ast.StructType); ok {
					slog.Info("[openapi] GenerateSchema: found qualified struct in TypeIndex", "typeName", typeName)
			...
			if ts, pkg := sg.typeIndex.LookupUnqualifiedType(typeName); ts != nil {
				if structType, ok := ts.Type.(*ast.StructType); ok {
					slog.Info("[openapi] GenerateSchema: found unqualified struct in TypeIndex", "typeName", typeName, "package", pkg)
					built = sg.convertStructToSchema(structType)
				}
			}
		}
	...
	if built == nil {
		slog.Info("[openapi] GenerateSchema: TypeIndex lookup failed, using basic type mapping", "typeName", typeName)
...
func (sg *SchemaGenerator) GenerateSchemaOld(typeName string) *Schema {
	...
	if sg.typeIndex != nil {
		if schema, ok := sg.typeIndex.externalKnownTypes[typeName]; ok {
			slog.Info("[openapi] GenerateSchema: using typeIndex.externalKnownTypes registry", "typeName", typeName)
			return schema
		}
	}
	...
	if sg.typeIndex != nil {
		...
		if len(parts) == 2 {
			pkg, typ := parts[0], parts[1]
			slog.Info("[openapi] GenerateSchema: using typeIndex", "pkg", pkg, "typ", typ)

			if ts := sg.typeIndex.LookupType(pkg, typ); ts != nil {
				if structType, ok := ts.Type.(*ast.StructType); ok {
					slog.Info("[openapi] GenerateSchema: found struct in TypeIndex", "typeName", typeName)
	...
	slog.Info("[openapi] GenerateSchema: falling back to findAndParseType", "typeName", typeName)
	// Fallback to old method
	schema := sg.findAndParseType(typeName)
	if schema != nil {
		slog.Info("[openapi] GenerateSchema: found schema via findAndParseType", "typeName", typeName)
		sg.schemas[typeName] = schema
		if sg.typeIndex != nil {
	...
	slog.Info("[openapi] GenerateSchema: using basic type mapping (external or not found)", "typeName", typeName)
...
func (sg *SchemaGenerator) GetSchemas() map[string]Schema {
	slog.Info("[openapi] GetSchemas: returning all generated schemas", "count", len(sg.schemas))
...
func (sg *SchemaGenerator) findAndParseType(typeName string) *Schema {
	slog.Info("[openapi] findAndParseType: DEPRECATED - this should not be called with TypeIndex", "typeName", typeName)
...
func (sg *SchemaGenerator) searchLocalType(typeName string) *Schema {
	slog.Info("[openapi] searchLocalType: DEPRECATED - use TypeIndex instead", "typeName", typeName)
	return nil
}
...
func (sg *SchemaGenerator) searchInDirectory(dir, typeName string) *Schema {
	slog.Info("[openapi] searchInDirectory: called", "dir", dir, "typeName", typeName)
	files, err := filepath.Glob(filepath.Join(dir, "*.go"))
	if err != nil {
		slog.Info("[openapi] searchInDirectory: glob error", "err", err)
		return nil
	}

	for _, file := range files {
		slog.Info("[openapi] searchInDirectory: parsing file", "file", file)
		if schema := sg.parseTypeFromFile(file, typeName); schema != nil {
			slog.Info("[openapi] searchInDirectory: found schema in file", "file", file)
			return schema
		}
	}
	...
	if err != nil {
		slog.Info("[openapi] searchInDirectory: subdir glob error", "err", err)
		return nil
	}
	...
	for _, subdir := range dirs {
		if info, statErr := os.Stat(subdir); statErr == nil && info.IsDir() {
			...
			if strings.HasPrefix(info.Name(), ".") {
				slog.Info("[openapi] searchInDirectory: skipping hidden dir", "subdir", subdir)
				continue
			}
			slog.Info("[openapi] searchInDirectory: recursing into subdir", "subdir", subdir)
...
func (sg *SchemaGenerator) parseTypeFromFile(filePath, typeName string) *Schema {
	slog.Info("[openapi] parseTypeFromFile: called", "filePath", filePath, "typeName", typeName)
	fset := token.NewFileSet()
	node, err := parser.ParseFile(fset, filePath, nil, parser.ParseComments)
	if err != nil {
		slog.Info("[openapi] parseTypeFromFile: parse error", "err", err)
		return nil
	}
			...
			for _, spec := range genDecl.Specs {
				if typeSpec, isTypeSpec := spec.(*ast.TypeSpec); isTypeSpec && typeSpec.Name.Name == typeName {
					if structType, isStructType := typeSpec.Type.(*ast.StructType); isStructType {
						slog.Info("[openapi] parseTypeFromFile: found struct type", "typeName", typeName)
...
func (sg *SchemaGenerator) convertStructToSchema(structType *ast.StructType) *Schema {
	slog.Info("[openapi] convertStructToSchema: called")
	schema := &Schema{
		Type:       "object",
...
func (sg *SchemaGenerator) convertFieldType(expr ast.Expr) *Schema {
	slog.Info("[openapi] convertFieldType: called")
	switch t := expr.(type) {
	case *ast.Ident:
		slog.Info("[openapi] convertFieldType: Ident type", "name", t.Name)
	...
	case *ast.StarExpr:
		slog.Info("[openapi] convertFieldType: StarExpr (pointer) type")
		return sg.convertFieldType(t.X)
	case *ast.ArrayType:
		slog.Info("[openapi] convertFieldType: ArrayType")
		return &Schema{
			Type:  "array",
			Items: sg.convertFieldType(t.Elt),
		}
	case *ast.SelectorExpr:
		slog.Info("[openapi] convertFieldType: SelectorExpr", "sel", t.Sel.Name)
		// Handle qualified types like json.RawMessage, time.Time, etc.
		if ident, ok := t.X.(*ast.Ident); ok {
			typeName := ident.Name + "." + t.Sel.Name
			slog.Info("[openapi] convertFieldType: generating schema for qualified type", "typeName", typeName)
			return sg.GenerateSchema(typeName)
		}
	case *ast.MapType:
		slog.Info("[openapi] convertFieldType: MapType")
		return &Schema{Type: "object", AdditionalProperties: sg.convertFieldType(t.Value)}
	case *ast.InterfaceType:
		slog.Info("[openapi] convertFieldType: InterfaceType")
		return &Schema{Type: "object"}
	}

	slog.Info("[openapi] convertFieldType: unknown type, defaulting to object")
	return &Schema{Type: "object"}
}
